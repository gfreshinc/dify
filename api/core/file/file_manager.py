...
from core.model_runtime.entities import AudioPromptMessageContent, ImagePromptMessageContent, PdfPromptMessageContent
...
def to_prompt_message_content(f: File, /):
    ...
    match f.type:
        ...
        case FileType.DOCUMENT:
            ...
            if f.extension is None:
                raise ValueError("Missing file extension")
            if f.extension.lower().find("pdf") != -1:
                return PdfPromptMessageContent(data=encoded_string)
            else:
                return AudioPromptMessageContent(data=encoded_string, format=f.extension.lstrip("."))
        case _:
