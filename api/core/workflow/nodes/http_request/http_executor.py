...
class HttpExecutor:
        ...
        if node_data.body:
            ...
            if node_data.body.type in {"form-data", "x-www-form-urlencoded"}:
                ...
                if node_data.body.type == 'json' and self.params and self.params.get('is_include_sysfiles'):
                    ...
                    for key in variable_pool.system_variables:
                        if key.value == SystemVariableKey.FILES.value:
                            ...
                            if sysfiles:
                                ...
                                sysfiles = [
                                    {
                                        "type": filevar.type.value, 
                                        "transfer_method": filevar.transfer_method.value, 
                                        "url": filevar.preview_url
                                    }
                                    for filevar in sysfiles
                                ]
    ...
    def _validate_and_parse_response(self, response: httpx.Response) -> HttpExecutorResponse:
        ...
        if self.params and self.params.get('is_only_workflow_finished') \
            and 'text/event-stream' in executor_response.headers.get('content-type'):
            # 获取response.content
